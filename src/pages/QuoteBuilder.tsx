...
export default function QuoteBuilder() {
  ...
  const saveQuote = async (sendAfterSave = false) => {
    ...
    // Validate email for sending
    if (sendAfterSave && !customerEmail.trim()) {
      toast({
        title: "Customer email required",
        description: "Please enter the customer's email to send the quote",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      // If sending, use the send-quote edge function
      if (sendAfterSave) {
        toast({
          title: "Sending quote...",
        ...
        if (!sendResponse.ok) {
          throw new Error(sendResult.error || "Failed to send quote");
        }

        toast({
          title: "Quote sent!",
          description: `Email delivered to ${customerEmail}`,
        });
      ...
      toast({
        title: sendAfterSave ? "Failed to send quote" : "Error",
